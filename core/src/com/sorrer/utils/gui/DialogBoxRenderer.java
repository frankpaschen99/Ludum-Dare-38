...
public class DialogBoxRenderer {
	...
	/**
	 * @param speed
	 *            Milliseconds it takes per character
	 */
	...
	public void draw(SpriteBatch b, ShapeRenderer sr) {
		...
		for (int i = 0; i < this.curDialog.size() - 1; i++) {
			...
			text.draw(l, b, this.position.x + this.spacing.x / 2, (((this.position.y + this.size.y))
					- (text.getStringHeight(l) * line) - paddingVert / 2 - this.spacing.y / 2) - this.spacing.y * line);
			line++;
		}

		text.draw(this.curLine, b, this.position.x + this.spacing.x / 2,
				(((this.position.y + this.size.y)) - (text.getStringHeight(this.curLine) * line) - paddingVert / 2
						- this.spacing.y / 2) - this.spacing.y * line);
